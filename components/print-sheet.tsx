...
export function PrintSheet({ client, company }: { client?: Client; company: Company }) {
  ...
  return (
    <>
      ...
      <div className="no-print fixed top-0 left-0 right-0 z-50 flex items-center justify-between gap-3 border-b bg-white px-6 py-3 shadow-sm">
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold text-ink">
            {client ? `Ficha de Medidas — ${client.name}` : "Ficha de Medidas em Branco"}
          </span>
        </div>
        <div className="flex items-center gap-2">
          {!client && (
          ...
          <a
            href={client ? `/clientes/${(client as unknown as { id?: string }).id ?? ""}` : "/medicoes"}
            className="flex items-center gap-2 rounded-xl border border-sand bg-white px-3 py-2 text-sm font-semibold hover:bg-cream"
            title="Fechar"
          >
            <X className="h-4 w-4" />
          </a>
      ...
      {/* ── Folha A4 ── */}
      <div className="flex min-h-screen justify-center bg-[#f0f0f0] pt-[60px] pb-8 print:bg-white print:p-0 print:pt-0 print:pb-0">
        <article
