...
export function AppShell({ children, userName, companyName, roles=[] }: { children: React.ReactNode; userName: string; companyName: string; roles?: string[] }) {
  ...
  return <div className="min-h-screen lg:flex">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-ink text-white transition-all lg:sticky lg:translate-x-0", open && "translate-x-0", collapsed && "lg:w-[86px]") }>
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-6"><Link href="/dashboard" className="font-black tracking-tight leading-tight">{collapsed ? <span className="text-forest text-xl">M</span> : <><span className="text-white">MARQUINHOS</span><br/><span className="text-lime text-xs font-bold tracking-widest">CALHAS &amp; ESQUADRIAS</span></>}</Link><button className="lg:hidden" onClick={() => setOpen(false)}><X /></button></div>
