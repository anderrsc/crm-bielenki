...
export default async function CatchAll({ params, searchParams }: { params: Promise<{ path?: string[] }>; searchParams: Promise<{ q?: string; erro?: string }> }) {
  ...
  if (path[0] === "orcamentos" && path[1] === "calhas") {
    ...
    if(process.env.NEXT_PUBLIC_SUPABASE_URL){const db=await createClient();const [priceResult,clientResult]=await Promise.all([db.from("gutter_prices").select("id,product,category,item_type,thickness,cut_mm,unit,color,unit_price,install_price,notes,active").eq("active",true),db.from("clients").select("id,name,phone,city").eq("status","ativo").order("name")]);prices=(priceResult.data as GutterPrice[])??[];clients=(clientResult.data as QuoteClient[])??[];}
  ...
  if(path[0]==="orcamentos"&&path[1]&&path[2]==="editar"){
    const db=await createClient();const [quoteResult,gutterResult,priceResult,clientResult,specialResult]=await Promise.all([db.from("quotes").select("id,client_id,discount,freight,notes,valid_until,installation_deadline,status").eq("id",path[1]).single(),db.from("gutter_quotes").select("items:gutter_quote_items(category,item_type,product,thickness,cut,color,quantity,meters,unit_price,install_price)").eq("quote_id",path[1]).single(),db.from("gutter_prices").select("id,product,category,item_type,thickness,cut_mm,unit,color,unit_price,install_price,notes,active").eq("active",true),db.from("clients").select("id,name,phone,city").eq("status","ativo").order("name"),db.from("quote_items").select("product,description,unit,quantity,unit_price").eq("quote_id",path[1]).eq("item_type","especial")]);
